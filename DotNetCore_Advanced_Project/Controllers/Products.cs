...
namespace WebApplication4.Controllers
{
    ...
    public class Products : ControllerBase
    {
        ...
        public IActionResult GetProductsByCategory(string category)
        {
            ...
            return Ok(_context.GetProductsByCategory(category));
        }

        [HttpGet("OutOfStock")]
        public IActionResult GetOutOfStock()
        {
            return Ok(_context.GetOutOfStock());
        }

        [HttpGet("start={start}&end={end}")]
        public IActionResult GetProductsInRange(int start, int end)
        {
            return Ok(_context.GetProductsInRange(start, end));
        }

        [HttpGet("Categories")]
        public IActionResult GetCategories()
        {
            return Ok(_context.GetCategories());
        }
